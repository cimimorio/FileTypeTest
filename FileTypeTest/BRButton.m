...
- (void)drawRect:(NSRect)dirtyRect {
    ...
    NSBezierPath * bezier = [NSBezierPath bezierPathWithRoundedRect:rectFill xRadius:radius yRadius:radius];
    [self.effectBackgroundColor setFill];
    [bezier fill];
	
	if ([self.textString isEqualToString:@"Scan"]) {
		self.layer.borderWidth = 3.0;
	}else{
		self.layer.borderWidth = 2.0;
	}
	
	self.layer.borderColor = [NSColor colorWithRed:12/255.0 green:154/255.0 blue:247/255.0 alpha:1.0f].CGColor;
